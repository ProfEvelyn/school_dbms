...
class StudentManagementApp:
    def __init__(self):
        ...
        # GUI elements
        # GUI elements
        self.label_id = tk.Label(self.root, text="Student ID:", bg="#3498db", fg="white")
        ...
        self.label_name = tk.Label(self.root, text="Student Name:", bg="#3498db", fg="white")
        ...
        self.label_cred = tk.Label(self.root, text="Total Credits:", bg="#3498db", fg="white")
        ...
        # Buttons
        self.btn_insert = tk.Button(self.root, text="Insert Student", command=self.insert_student, bg="#8e44ad", fg="white")
        self.btn_insert.grid(row=4, column=0, columnspan=2, pady=10)

        self.btn_search = tk.Button(self.root, text="Search Student", command=self.search_student, bg="#8e44ad", fg="white")
        self.btn_search.grid(row=5, column=0, columnspan=2, pady=10)

        self.btn_delete = tk.Button(self.root, text="Delete Student", command=self.delete_student, bg="#8e44ad", fg="white")
        self.btn_delete.grid(row=6, column=0, columnspan=2, pady=10)

        self.btn_list = tk.Button(self.root, text="List All Students", command=self.list_students, bg="#8e44ad", fg="white")
        self.btn_list.grid(row=8, column=0, columnspan=2, pady=10)

        self.btn_delete = tk.Button(self.root, text="Update Student", command=self.update_student, bg="#8e44ad", fg="white")
        self.btn_delete.grid(row=7, column=0, columnspan=2, pady=10)

        self.tree = ttk.Treeview(self.root, columns=("ID", "Name", "Department", "Total Credits"), show="headings")
        self.tree.heading("ID", text="ID")
        self.tree.heading("Name", text="Name")
        self.tree.heading("Department", text="Department")
        self.tree.heading("Total Credits", text="Total Credits")
    ...
    def insert_student(self):
        ...
        if not student_id.isdigit() or len(student_id) != 5:
            messagebox.showerror("Error", "Student ID must be a 5-digit numeric value.")
            return

        # Validate total credit
        if not tot_cred.isdigit() or int(tot_cred) < 0:
            messagebox.showerror("Error", "Total credits must be a non-negative numeric value.")
            return

        # Check for empty values
        if not student_name or not dept_name:
